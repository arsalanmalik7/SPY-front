...
const Login = () => {
  ...
  useEffect(() => {
    if (location.state?.successMessage) {
      setWelcomeMessage(location.state.successMessage);
      setShowWelcome(true);
    }
    ...
    if (validateForm()) {
      ...
      try {
        ...
        if (response?.user?.role === "employee") {
          setTimeout(() => {
            navigate('/my-lessons', { replace: true, state: { successMessage: 'Login successful' } });
          }, 1000);
        } else {
          setTimeout(() => {
            navigate('/dashboard', { replace: true, state: { successMessage: 'Login successful' } });
          }, 1000);
        }
      } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            <Alert
              ...
              icon={<svg className="w-6 h-6 mr-2 text-red-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" fill="none" /><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4m0 4h.01" /></svg>}
            >
              {errorMessage}
            </Alert>
          </Snackbar>
          ...
          <Snackbar
            ...
            anchorOrigin={{ vertical: 'top', horizontal: 'right' }}
          >
            <Alert onClose={() => setShowWelcome(false)} severity="success" sx={{ width: '100%' }}>
              {welcomeMessage}
            </Alert>
          </Snackbar>
          ...
          <div className="flex justify-end">
            <Link to="/forgot-password" className="text-sm sm:text-base text-icon hover:underline">
          ...
          <div className="mt-4 text-center">
            <p className="text-sm sm:text-base text-gray-600">
              ...
              <Link to="/signup" className="font-medium text-icon ">
                Sign up
              </Link>
            </p>
          </div>
