...
export default function ManageSubscription() {
    ...
    const freePlan = plans.find(p => p.unit_amount === 0 && p?.product?.name === "Free trial");
    const singleLocationPlan = plans.find(p => p.unit_amount === 7999); // $79.99/month
    const multiLocationPlan = plans.find(p => p.unit_amount === 0 && p?.product?.name === "Multi Location");
    ...
    const handleCheckout = async (planId, quantity = 1, amountDollars, type) => {
        ...
        try {
            ...
            const res = await SubscriptionService.createCheckoutSession(planId, quantity, amountDollars, isFreeTrial);
            const data = res;
            if (data.id) {
    ...
    const pricePerLocation = singleLocationPlan ? singleLocationPlan.unit_amount / 100 : 79.99; // Stripe amount is in cents
    ...
    return (
        <div className="max-w-7xl mx-auto p-2 md:p-6 w-full">
            ...
            {!subscription?.current_subscription?.status && subscription?.current_subscription?.status !== undefined && userRole !== "super_admin" ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg shadow-card p-6 flex flex-col justify-between">
                        ...
                        <Button className="bg-primary text-white w-full mt-2" onClick={() => handleCheckout(freePlan?.id, 1, 0, "free")}>Choose Free</Button>
                    </div>
                    {/* Single Location Plan */}
                    <div className="bg-white border border-gray-200 rounded-lg shadow-card p-6 flex flex-col justify-between">
                        <div>
                            <div className="text-lg font-semibold mb-2">Single Location</div>
                            <div className="text-2xl font-bold mb-2">$79.99<span className="text-base font-normal">/month</span></div>
                            <ul className="text-sm text-gray-700 mb-4 list-disc pl-5 space-y-1">
                                <li>1 location</li>
                        ...
                        <Button className="bg-primary text-white w-full mt-2" onClick={() => singleLocationPlan && handleCheckout(singleLocationPlan.id, 1, singleLocationPlan?.amount_decimal, "single")}>Subscribe</Button>
                    </div>
                    {/* Multi-Location Plan */}
                    <div className="bg-white border border-gray-200 rounded-lg shadow-card p-6 flex flex-col justify-between">
                        <div>
                            <div className="text-lg font-semibold mb-2">Multi-Location</div>
                            <div className="text-2xl font-bold mb-2">${Number(multiLocationCount * 79.99).toFixed(2)}<span className="text-base font-normal">/month</span></div>
                            <ul className="text-sm text-gray-700 mb-4 list-disc pl-5 space-y-1">
                        ...
                        <Button className="bg-primary text-white w-full mt-2" onClick={() => setMultiLocationModalOpen(true)}>Subscribe</Button>
                    </div>
                </div>
            ) : null}
            {subscription?.current_subscription?.status && userRole !== "super_admin" ? (
                <>
                    ...
                    <div className="flex items-stretch mb-4">
                        ...
                        <div className="flex-1 bg-[#FDECEC] border border-[#F9CFCF] rounded-r-md px-4 py-3 flex items-center">
                            ...
                            <span className="text-sm text-[#C1121F]">Your subscription will expire in {remainingDaysInExpiry} days</span>
                        </div>
                    </div>
                        ...
                        <div className="bg-white text-left border border-gray-200 rounded-lg shadow-card p-4 flex flex-col justify-between min-h-[100px]">
                            <div>
                                ...
                                <div className='flex flex-col md:flex-row md:items-center md:justify-between mb-2 gap-2'>
                                    ...
                                    <span className='text-base font-normal'>{remainingDaysInExpiry} days left</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {userRole === "super_admin" && transactions ?
            ...
            <Modal isOpen={multiLocationModalOpen} onClose={() => setMultiLocationModalOpen(false)}>
                <div className="p-6">
                    <h2 className="text-lg font-semibold mb-4">Select Number of Locations</h2>
                    <input
                        type="number"
                        min={2}
                        max={20}
                    ...
                    />
                    <div className="mb-4">Total: <span className="font-bold">${Number(multiLocationCount * pricePerLocation).toFixed(2)}/month</span></div>
                    <div className="flex gap-2">
                        <Button className="bg-primary text-white flex-1" onClick={async () => {
                            setMultiLocationModalOpen(false);
                            if (multiLocationPlan) {
                                const amountDollars = (multiLocationCount * pricePerLocation).toFixed(2)
                                await handleCheckout(multiLocationPlan?.product?.id, multiLocationCount, amountDollars, "multi");
                            }
                        }}>
                            Confirm
                        </Button>
                        <Button className="bg-gray-300 flex-1" onClick={() => setMultiLocationModalOpen(false)}>Cancel</Button>
                    </div>
                </div>
            </Modal>
