  ...
  <Card className={`mb-4 w-full ${locked ? 'bg-[#f8f3e7] opacity-80' : 'bg-white'}`}>
    ...
    <div className="space-y-2 mt-2">
      {chapters.map((ch, idx) => {
        return (<div
          key={idx}
          className={`flex items-center justify-between rounded-lg px-2 py-2 sm:px-3 sm:py-2 mb-1 ${ch.status === 'completed'
        ...
        >
          <div className={`flex flex-col text-sm sm:text-base text-left ${ch.status === 'locked' ? 'text-gray-500' : ''}`}>
            <span className={`font-medium text-left ${ch.status === 'locked' ? 'text-gray-500' : ''}`}>
              Chapter {ch.chapter}
            </span>
            <span className={`ml-0 mt-0.5 text-xs sm:text-sm font-normal text-left ${ch.status === 'locked' ? 'text-gray-500' : ''}`}>{ch.title}</span>
          </div>
          <div>
            <span className='p-2'>{ch.questions && Array.isArray(ch.questions) ? ` ${ch?.attemptedQuestions?.length}/${ch.questions.length}` : ''}</span>
            <ChapterStatus status={ch.status} onStart={ch.status === 'start' ? () => onStartQuiz(type, unit, ch.title) : undefined} />
          </div>
        </div>)
      })}
    </div>
...
export const MyLessons = () => {
  ...
  const filteredFoodLessons = lessons.filter(lesson => lesson.category === 'food');
  const filteredWineLessons = lessons.filter(lesson => lesson.category === 'wine');


  const processedFoodLessons = filteredFoodLessons
    ?.map((lesson, idx) => {
      ...
      if (idx === 0) {
        ...
        if (filteredFoodLessons[idx - 1].status !== 'Completed' || anyFoodFailed) {
          status = 'locked';
  ...
  const processedWineLessons = filteredWineLessons
    ?.map((lesson, idx) => {
      ...
      if (idx === 0) {
        ...
        if (filteredWineLessons[idx - 1].status !== 'Completed' || anyWineFailed) {
          status = 'locked';
        } else if (status === 'Completed') {
          status = 'completed';
        } else {
  ...
  const getTabContent = (type) => {
    ...
    return (
      <>
        {(type === 'all' || type === 'food') && groupFoodLessons[0].length !== 0 && (
          <>
        ...
        {(type === 'all' || type === 'wine') && groupWineLessons[0].length > 0 && (
          <>
  ...
  function handleStartQuiz(type, unit, chapter,) {
    ...
    if (foundLesson) {
      ...
      const attemptedIds = (foundLesson.attemptedQuestions || []).map(aq => aq.uuid);
      // Filter out questions whose uuid is in attemptedIds
      const unattemptedQuestions = foundLesson.questions.filter(q => !attemptedIds.includes(q.uuid));
