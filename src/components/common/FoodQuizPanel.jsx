...
const FoodQuizPanel = ({
  ...
  const handleNext = () => {
    ...
    setProgress((prev) =>
      Math.min(prev + (nextIndex - currentIndex), questions.length)
    );
  };
  ...
  const handleCompleteLesson = async () => {
    ...
    if (
      ...
      if (!updatedAnswers[questions.length - 1]) {
        updatedAnswers[questions.length - 1] = [
          questions[questions.length - 1].options.indexOf("None"),
        ];
      }
    ...
    try {
      ...
      const correctAnswers = questions.reduce((total, q, idx) => {
        ...
        if (
          ...
          return (
            total +
            (userValues.length === q.correct.length &&
            q.correct.every((c) => userSet.has(c))
              ? 1
              : 0)
          );
        }
