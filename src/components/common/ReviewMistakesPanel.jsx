  ...
  return (
    <div className="fixed inset-0 z-[200] w-full h-full bg-[#fcf7ec] overflow-auto">
      ...
      <div className="relative w-full max-w-8xl mx-auto pt-8 pb-2 px-4 sm:px-8">
        <h2 className="text-lg sm:text-xl font-semibold text-black mb-1 text-left">
          Review Mistakes
        </h2>
        ...
        <button
          onClick={onClose}
          className="absolute top-8 right-4 sm:right-8 text-gray-500 hover:text-red-500 p-1 rounded-full"
        >
          <X size={24} />
        </button>
      </div>
            ...
            <div className="px-4 py-4">
              <div className="text-base sm:text-lg font-semibold text-black mb-2">
                {(() => {
                  ...
                  if (lastIndex < question.length) {
                    parts.push(
                      <span key={key++}>{question.slice(lastIndex)}</span>
                    );
                  }
                  return parts;
                })()}
              ...
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-sm sm:text-base text-[#C1121F] font-semibold">
                  Your answer:{" "}
                  {currentMistake?.userAnswer?.startsWith("http") ||
                  currentMistake?.userAnswer?.startsWith("/uploads/") ? (
                    <img
                      className="mx-auto my-2 block max-h-52"
                      src={
                        currentMistake?.userAnswer?.startsWith("http")
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm sm:text-base text-[#FFD600] font-semibold">
                  Correct answer:{" "}
                  {currentMistake?.correctAnswer?.startsWith("http") ||
                  currentMistake?.correctAnswer?.startsWith("/uploads/") ? (
                    <img
                      className="mx-auto my-2 block max-h-52"
                      src={
                        currentMistake?.correctAnswer?.startsWith("http")
